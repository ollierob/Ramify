...
import {MonthDayWordsFormatter} from "./DateFormatter";

test("formatDateRange_WholeYear", () => {

    const year: DateRange.AsObject = {
        approximate: false,
        earliest: {year: 1900, month: 1, day: 1},
        latest: {year: 1900, month: 12, day: 31}
    };
...
test("formatDateRange_InMonth", () => {

    const month: DateRange.AsObject = {
        approximate: false,
        earliest: {year: 1900, month: 2, day: 1},
        latest: {year: 1900, month: 2, day: 20}
    };

    const words: Partial<PrefixWords> = {in: "in ", between: "between "};

    expect(formatDateRange(month, MonthDayWordsFormatter, words)).toBe("in Feb");
