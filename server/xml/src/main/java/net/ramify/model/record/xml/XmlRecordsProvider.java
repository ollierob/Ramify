...
@XmlTransient
@Singleton
class XmlRecordsProvider implements RecordsProvider {

    private static final Logger logger = LoggerFactory.getLogger(XmlRecordsProvider.class);
    ...
    private PlaceIndex index() {
        ...
        final Multimap<PlaceId, RecordSetId> index = xmlFiles.keySet()
                ...
                .flatMap(XmlRecordsProvider::streamRecordPlaces)
                .parallel()
    ...
    private class PlaceIndex implements RecordsByPlaceIndex {
        ...
        public Records get(final PlaceId id) {
            ...
            return () -> ids.stream()
                    .map(XmlRecordsProvider.this::get)
